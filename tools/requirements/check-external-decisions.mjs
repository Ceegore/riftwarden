...
function validateValue(decision) {
  ...
  if (rule.type === 'not_placeholder') {
    ...
    if ((rule.forbiddenDomains ?? []).map(String).map((entry) => entry.toLowerCase()).includes(domain)) return 'email domain is forbidden';
  } else if (rule.type === 'https_url') {
    try {
      const url = new URL(value);
      if (url.protocol !== 'https:') return 'URL is not HTTPS';
      if ((rule.forbiddenHosts ?? []).map(String).map((entry) => entry.toLowerCase()).includes(url.hostname.toLowerCase())) return 'URL host is forbidden';
    } catch {
