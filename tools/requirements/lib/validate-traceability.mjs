...
export async function validateTests(root) {
  ...
  for (const id of duplicates(tests.map((test) => test.id))) {
...
export async function validateTraceability(root, mode, requirements, tests, normRecords) {
  ...
  const testIds = new Set(tests.map((entry) => entry.id));
  const normIds = new Set(normRecords.filter((entry) => entry.status === 'active').map((entry) => entry.id));
  ...
  for (const [index, link] of links.entries()) {
    ...
    if (!Array.isArray(link?.testIds) || link.testIds.length === 0) {
      ...
      link.testIds.forEach((testId, testIndex) => {
    ...
    if (!Array.isArray(link?.phaseIds) || link.phaseIds.length === 0) {
      ...
      link.phaseIds.forEach((phaseId, phaseIndex) => {
  ...
  for (const id of duplicates(links.map((link) => link.requirementId))) {
  ...
  const linkedRequirements = new Set(links.map((link) => link.requirementId));
  ...
  const linkedTests = new Set(links.flatMap((link) => link.testIds ?? []));
