import {
  fileExists, fileSize, issue, pushEnum, pushRequiredString,
...
/**
 * Validates a single heading record.
 * @param {any} record Heading record.
...
/**
 * Validates the source manifest and its derived artifacts.
 * @param {string} root Repository root.
 * @param {string} mode Validation mode ('draft' or 'gate').
 * @returns {Promise<{issues: Issue[], manifest: any, headings: any}>}
 */
export async function validateSource(root, mode) {
  ...
  if (!(await fileExists(root, headingsPath))) {
    ...
    if (!Array.isArray(headings?.headings)) {
      ...
      /** @type {(record: any, index: number) => void} */
      const checkRecord = (record, index) => validateHeadingRecord(record, index, issues);
      headings.headings.forEach(checkRecord);
      ...
      const isUnreviewed = (record) => record.reviewStatus !== 'verified';
      if (mode === 'gate' && headings.headings.some(isUnreviewed)) {
  ...
  if (findingsPath && await fileExists(root, findingsPath)) {
    ...
    const isOpen = (entry) => entry.status === 'open';
    const open = (findings?.findings ?? []).filter(isOpen);
    if (open.length > 0) {
