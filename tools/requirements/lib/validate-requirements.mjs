...
/**
 * Validates the source locator of a requirement.
 * @param {any} requirement Requirement record.
 * @param {string} base Base JSON path.
...
/**
 * Validates the verification array of a requirement.
 * @param {any} requirement Requirement record.
 * @param {string} base Base JSON path.
 * @param {Issue[]} issues Issue accumulator.
...
/**
 * Validates a single requirement record.
 * @param {any} requirement Requirement record.
 * @param {string} base Base JSON path.
 * @param {Issue[]} issues Issue accumulator.
 * @param {string} mode Validation mode.
...
function validateRequirement(requirement, base, issues, mode) {
  ...
  if (!Array.isArray(requirement?.ownerPhases) || requirement.ownerPhases.length === 0) {
    ...
    /** @type {(phaseId: any, index: number) => void} */
    const pushPhase = (phaseId, index) => {
      pushId(issues, phaseId, ID_PATTERNS.phase, `${base}.ownerPhases[${index}]`, 'REQ_OWNER_PHASE');
    };
    requirement.ownerPhases.forEach(pushPhase);
  }
...
/**
 * Validates that every chapter is either covered by a requirement or a context-only disposition.
...
/**
 * Validates the requirements register.
 * @param {string} root Repository root.
 * @param {string} mode Validation mode ('draft' or 'gate').
 * @returns {Promise<{issues: Issue[], requirements: any[], requirementFiles: string[]}>}
 */
