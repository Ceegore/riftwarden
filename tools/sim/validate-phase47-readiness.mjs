...
const root = resolve(process.argv[2] ?? '.'); const p = join(root, 'contracts', 'phase47', 'phase47-readiness.expected.json');
function rj(x) { if (!existsSync(x)) return null; try { return JSON.parse(readFileSync(x, 'utf8')); } catch { return null; } }
if (!existsSync(p)) { console.log(JSON.stringify({ schemaVersion: 1, phase: 47, gate: 'G47', status: 'BLOCKED', blockers: ['P47_CONTRACT_MISSING'] })); process.exit(2); }
const c = rj(p); const b = []; const s = [];
const chk = existsSync(join(root, c.evidence.checklist));
if (!chk) b.push('P47_CHECKLIST'); else s.push({ id: 'checklist' });
for (const hb of c.hardBlockers) { if (!b.includes(hb)) b.push(hb); }
const r = { schemaVersion: 1, phase: 47, gate: 'G47', status: 'BLOCKED', blockers: b, satisfied: s };
