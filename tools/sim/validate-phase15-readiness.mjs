...
const contractPath = join(root, 'contracts', 'phase16', 'phase15-readiness.expected.json');
...
if (!existsSync(contractPath)) {
  console.log(JSON.stringify({ schemaVersion: 1, phase: 16, gate: 'G15', status: 'BLOCKED', blockers: ['P16_G15_NOT_REPRODUCED'], errors: ['phase15-readiness contract missing'] }, null, 2));
  process.exit(2);
}
...
// Mass-sim evidence (Node reference performance for the Phase 15 kernel).
const massSim = readJson(join(root, contract.evidence.massSim.path));
if (!massSim) {
  blockers.push('P16_G15_MASSSIM_MISSING');
...
// Cross-runtime evidence (Phase 15 movement trace: Node reference + desktop engines).
...
if (!section || section.runtimes?.node?.status !== 'REFERENCE') {
  blockers.push('P16_G15_CROSSRUNTIME_MISSING');
} else {
  ...
  if (devicesPending.length > 0) blockers.push('P16_G15_WEBVIEWS_NOT_RUN');
...
console.log(JSON.stringify({ schemaVersion: 1, phase: 16, gate: 'G15', status, blockers, satisfied }, null, 2));
