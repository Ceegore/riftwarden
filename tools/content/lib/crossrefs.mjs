...
function refsFor(type,e){
 ...
 if(type==="encounter"){for(const x of e.enemySlots??[])add(x.unitId,["unit"],"enemySlots.unitId");add(e.rewardTableId,["rewardTable"],"rewardTableId");for(const x of e.reinforcementWaves??[])add(x.encounterId,["encounter"],"reinforcementWaves.encounterId");for(const x of e.modifierIds??[])add(x,["modifier"],"modifierIds");}
