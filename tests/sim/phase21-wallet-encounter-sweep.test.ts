...
/** Commits the sweep's victory on a real expedition node of the encounter's family and audits the grant. */
function auditGrant(entry: Readonly<ContentEncounterEntry>, kinds: readonly string[], bounty: number): void {
  ...
  const walked = walkToFamily(familySeedFor(family), family);
  if (walked === null) throw new Error(`no ${family} node for the audit`);
  let exp = walked;
  const nodeId = exp.currentNodeId;
  exp = exp.enter(`sw-enter-${entry.id}`);
  ...
  expect(committed.state.gold - goldBefore).toBe(base + bounty);
  ...
  const second = committed.act({ transactionId: `sw-engage2-${entry.id}`, nodeId, action: 'ENGAGE', completedKinds: kinds });
  expect(second.state.ledger[`sw-engage2-${entry.id}`]?.status).toBe('REJECTED');
