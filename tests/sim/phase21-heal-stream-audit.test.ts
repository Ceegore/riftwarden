 ...
 *      halves the factor (5000), overheal at full HP discards (delta 0 →
 *      objective still gains max(1, 0)=1). Every `HealApplied` payload field
 *      is asserted.
 ...
 *      equals Σ max(1, delta) over heals applied at ticks ≤ t-2 (the stage-L
 *      previous-tick fold latency) — reproduced with the SAME delta the oracle
 *      just verified.
...
function run(mode: Mode): RunResult {
  ...
  const systems: readonly KernelSystem[] = Object.freeze([
    ...createPhase17Systems({
      ...
      targeting: { focusTargetId: { unit_p: SHIELD_OBJ, unit_p2: IMMUNE_OBJ, unit_e1: 'unit_p' } },
      basicAttack: { parameters: { unit_p: attack(300), unit_p2: attack(300), unit_e1: attack(60) } },
    }),
    ...createPhase21Systems({ bossObjects: [shieldObject, immuneObject], modifiers: [LIFESTEAL], objectives: [GATE] }),
  ]);
  ...
  let state: BattleModel = battle({
    ...
    tick: tick(0),
    entities: Object.freeze([a, b, enemy, shieldBody, immuneBody]),
  ...
  // Sum of max(1, delta) per heal tick — matched to the fold latency ≤ t-2.
  ...
  for (let t = 0; t < 200; t++) {
    ...
    // so the kernel progress equals Σ max(1, delta) over heals with tick ≤ t-2.
  ...
  return {
    ...
    objectiveTotal: healByTick.reduce((sum, h) => sum + Math.max(1, h.delta), 0),
    shieldProcs: healByTick.filter((h) => h.delta === (mode === 'halved' ? mulDivRound(HEAL_RAW, COLLAPSE_HEAL_FACTOR_BPS, 10000) : HEAL_RAW)).length,
