# WP P00-T02c – Requirements Extraction Chunk C (chapters 59-87)

## Identity
- Version: 1
- Phase: PHASE-00
- Phase Contract Hash: a5d4670e8287640fb43a72cb4e307ae0e3951245053cd0dad4b90370c388f8bc
- Idempotency Key: rw-p00-t02c-v1-20260722
...
## Goal
Extract ALL normative requirements from GDD V5 chapters 59..87 into atomic REQ
...
docs/requirements/requirements/_staging/chunk-c.json
(shape: {"schemaVersion":"1.0","chunk":"c","chapterRange":{"lo":59,"hi":87},"requirements":[...],"contextOnly":[{"chapter":N,"reason":"..."}]})

## Read Inputs
- docs/requirements/generated/source-structure.json (block text for chapters 59-87)
...
## In Scope
- For EACH chapter 59..87: read its blocks from source-structure.json.
- Extract every normative statement as an atomic REQ (split compound clauses).
- Assign TEMP id REQ-TEMP-C-0001.. per record (merge assigns final category IDs).
...
## Out of Scope
- Chapters outside 59-87 (other chunks). Final REQ-ID assignment (merge). Category files. Norms (T03). Traceability (T05).
...
| ID | Criterion |
|---|---|
| AC1 | chunk-c.json exists, valid JSON |
| AC2 | every chapter 59..87 has >=1 REQ or a context_only entry |
...
| T2 | git-add | ['docs/requirements/requirements/_staging/chunk-c.json'] |
| T3 | git-commit | ['-m','feat(requirements): extract REQs chapters 59-87 (P00-T02c)'] |
