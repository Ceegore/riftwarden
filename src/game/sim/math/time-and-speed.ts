...
export function secondsToTicks(value: string, ensurePositiveMinimum = false): TickConversion {
  ...
  return { ticks: tick(ticksValue), deviationMicros, ...(deviationMicros > 10_000 ? { warningCode: 'P12_SECONDS_PRECISION_WARNING' as const } : {}) };
}
...
export function controlDurationTicks(base: Tick, resistance: BasisPoints, boss = false): Tick {
  ...
  const remaining = basisPoints(10_000 - resistance,0,10_000);
