...
export function snapshotPayload(state:BattleModel):Omit<BattleSnapshotData,'checksum'>{
  ...
  if(state.previousTickEvents!==undefined)extras['previousTickEvents']=Object.freeze(state.previousTickEvents.map((e)=>Object.freeze(e.amount===undefined?{type:e.type,sourceId:e.sourceId,targetIds:Object.freeze([...e.targetIds])}:{type:e.type,sourceId:e.sourceId,targetIds:Object.freeze([...e.targetIds]),amount:e.amount})));
  ...
  if(state.bossPhaseSecondary!==undefined)extras['bossPhaseSecondary']=Object.freeze({entityId:state.bossPhaseSecondary.entityId,bossId:state.bossPhaseSecondary.bossId,phaseId:state.bossPhaseSecondary.phaseId,transition:state.bossPhaseSecondary.transition===null?null:Object.freeze({from:state.bossPhaseSecondary.transition.from,to:state.bossPhaseSecondary.transition.to,startTick:state.bossPhaseSecondary.transition.startTick,commitTick:state.bossPhaseSecondary.transition.commitTick}),visited:Object.freeze([...state.bossPhaseSecondary.visited].sort(asciiCompare)),invulnerableUntilTick:state.bossPhaseSecondary.invulnerableUntilTick});
